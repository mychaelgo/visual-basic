...
Private Sub Form_Load()
    ...
    cboSP.InsertItemByRecordset GetRST("select supplierid,companyname from suppliers order by supplierid"), True, True
    cboSP.TextColumn = 1 ' Display >> CompanyName
    cboSP.ColumnWidth(1) = 0 ' Hidden SupplierID
    cboSP.FitColumnWidth 2
    
    CboCtg.InsertItemByRecordset GetRST("select categoryid,categoryname from categories order by categoryid"), True, 1
    CboCtg.TextColumn = 1 ' Display Category NAme
    CboCtg.ColumnWidth(1) = 0 ' Hidden CategoryID
    CboCtg.FitColumnWidth 2

    If IsNew Then
        MyADODC1.OpenRecordset "select * from products where productid=-1", ADOCN, txtData   ' open empty record
    Else
        MyADODC1.OpenRecordset "select * from products where productid=" & KeyValue, ADOCN, txtData   ' open empty record
    End If
